...
from data import location_dawson_set_001  # <-- My primary example data set.
...
data_set = ConvertCsvResultsIntoDictionary(location_dawson_set_001).data

# Feed your raw data to the measurements object to calculate the relevant statistics.
baseline_measurements = Measurements(
    data=
    data_set[2498]["response_times"] +
    data_set[2940]["response_times"] +
    data_set[3461]["response_times"] +
    data_set[3979]["response_times"]
)
benchmark_measurements = Measurements(
    data=
    data_set[4418]["response_times"]
)
